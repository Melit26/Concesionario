...
public interface VentaRepository extends JpaRepository<Venta, Long> {
    ...
    @Query(
        value = "select distinct venta from Venta venta left join fetch venta.cliente",
        countQuery = "select count(distinct venta) from Venta venta"
    )
    Page<Venta> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct venta from Venta venta left join fetch venta.cliente")
    List<Venta> findAllWithToOneRelationships();

    @Query("select venta from Venta venta left join fetch venta.cliente where venta.id =:id")
